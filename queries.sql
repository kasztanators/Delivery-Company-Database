...
SELECT name,surname, COUNT(Packages.packageID) AS 'Number of packages'
	...
	Orders.userREF = Users.userID AND Packages.is_fragile =1
	GROUP BY Users.name,surname
	ORDER BY name


--2Find the average weight of packages sent to a specific city:
SELECT AVG(weight) AS 'average package weight', COUNT(PackageID) AS 'No of packages to city',city
FROM Packages
...
--5Find the most commonly used delivery unit that was used for a package delivery:
		SELECT Delivery_units.unitID, COUNT(Delivery_units.unitID) AS 'frequency',city
		FROM Delivery_units
		...
		GROUP BY Delivery_units.unitID, Addresses.city
...
SELECT Users.user_name ,COUNT(Packages.packageID) AS 'number Packages over average'
FROM Packages
JOIN Orders ON Orders.orderID = Packages.orderREF
JOIN Users ON Users.userID = Orders.userREF
WHERE Packages.weight >(SELECT AVG(weight) FROM Packages)
GROUP BY(user_name)
...
SELECT user_name, COUNT(packageID)AS 'Number of Packages'
    FROM Users , Packages,Orders 
...
-- 10 number of deliverymen which can drive available vehicles
 ...
 JOIN Vehicles ON Vehicles.vehicleID = Driverlogs.vehicleREF
 WHERE Vehicles.is_available = 1
 
 --11 POD KTOREGO MIASTA NAJCZESCIEJ PRZYJEZZDA£ POJAZD O TAKIEJ REJSTRACJI
 SELECT city , register_plate
 FROM Addresses
